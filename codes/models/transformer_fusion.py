...
class TransformerFusion(nn.Module):
    """
    Transformer-based fusion network for ECG and ME features.
    """
    def __init__(self, 
                 ECG_input_dim, me_input_dim,
                 d_model=128, 
                 num_heads=8, 
                 num_layers=2, 
        ...
        """
        Args:
            ECG_input_dim (int): Input dimension for ECG features.
        ...
        self.ECG_embedding = nn.Linear(ECG_input_dim, d_model)
        self.ECG_positional_encoding = PositionalEncoding(d_model, dropout)
        self.ECG_transformer = TransformerEncoder(
        ...
        self.cross_ECG_to_me = CrossTransformer(d_model, num_heads, num_layers, dropout)
        self.cross_me_to_ECG = CrossTransformer(d_model, num_heads, num_layers, dropout)
    ...
    def forward(self, ECG_tokens, me_tokens):
        """
        Args:
            ECG_tokens (torch.Tensor): ECG input tokens of shape (batch_size, seq_len_ECG, ECG_input_dim).
        ...
        """
        # ECG Encoding
        ECG_tokens = self.ECG_embedding(ECG_tokens)  # Shape: (batch_size, seq_len_ECG, d_model)
        ECG_tokens = self.ECG_positional_encoding(ECG_tokens)
        ECG_encoded = self.ECG_transformer(ECG_tokens)  # Shape: (batch_size, seq_len_ECG, d_model)
        ...
        ECG_to_me = self.cross_ECG_to_me(ECG_encoded, me_encoded)  # Shape: (batch_size, seq_len_me, d_model)
        me_to_ECG = self.cross_me_to_ECG(me_encoded, ECG_encoded)  # Shape: (batch_size, seq_len_ECG, d_model)
        
        # Fusion of Cross-modal Features
        ECG_fusion = ECG_to_me.mean(dim=1)  # Global average pooling over sequence dimension
        me_fusion = me_to_ECG.mean(dim=1)
        fusion_features = torch.cat([ECG_fusion, me_fusion], dim=-1)  # Shape: (batch_size, 2 * d_model)
