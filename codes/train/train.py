...
def train_epoch(model, data_loader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, (ECG_data, me_data, labels) in enumerate(data_loader):
        ...
        outputs = model(ECG_features, me_features)
        loss = criterion(outputs, ECG_features, me_features, labels)
        loss.backward()
        optimizer.step()
        total_loss += loss.item()

        if batch_idx % config['train']['log_interval'] == 0:
            print(f"Train Epoch: {epoch} [{batch_idx * len(ECG_data)}/{len(data_loader.dataset)} "
...
def validate_epoch(model, data_loader, criterion, device, epoch):
    ...
    with torch.no_grad():
        for ECG_data, me_data, labels in data_loader:
            ECG_data, me_data, labels = ECG_data.to(device), me_data.to(device), labels.to(device)
            ECG_features = ECG_feature_extractor(ECG_data)
            me_features = me_feature_extractor(me_data)
            outputs = model(ECG_features, me_features)
...
def create_loso_split(dataset, subject_idx):
    ...
    Args:
        dataset: Combined ECG and ME dataset with subject information
...
def main():
    ...
    # Load datasets
    ECG_dataset = ECGDataset(config['data']['ECG_path'])
    me_dataset = MEDataset(config['data']['me_path'])
    
    # Assuming datasets have been combined with subject information
    combined_dataset = list(zip(ECG_dataset, me_dataset))  # This needs proper implementation based on your data structure
