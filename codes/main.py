...
from models.ECG_feature_extractor import ECGFeatureExtractor
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Multimodal ME Spotting")
...
def initialize_models_and_optimizers(config, device):
    ...
    ECG_feature_extractor = ECGFeatureExtractor().to(device)
    ...
    optimizer = optim.Adam([
        {'params': ECG_feature_extractor.parameters()},
    ...
    return ECG_feature_extractor, me_feature_extractor, fusion_model, criterion, optimizer, scheduler
...
def train_with_loso(config, combined_dataset, device, log_dir_base):
    ...
    for fold, (train_idx, val_idx) in enumerate(loo.split(subject_ids)):
        ...
        ECG_feature_extractor, me_feature_extractor, fusion_model, criterion, optimizer, scheduler = \
...
def main():
    ...
    ECG_dataset = ECGDataset(config['data']['data/ECG'])
    ...
    combined_dataset = list(zip(ECG_dataset, me_dataset))  # Temporary; implement proper pairing
    ...
    if args.mode == 'train':
        ...
        # Prepare test data
        _, _, test_dataset = load_data_split(ECG_dataset, me_dataset)
        test_loader = DataLoader(test_dataset, batch_size=config['train']['batch_size'])
        
        # Initialize models
        ECG_feature_extractor, me_feature_extractor, fusion_model, criterion, optimizer, _ = \
            initialize_models_and_optimizers(config, device)
        ...
        fusion_model.eval()
        with torch.no_grad():
            for batch_idx, (ECG_data, me_data, labels) in enumerate(test_loader):
                ECG_data, me_data, labels = ECG_data.to(device), me_data.to(device), labels.to(device)
                ECG_features = ECG_feature_extractor(ECG_data)
                me_features = me_feature_extractor(me_data)
                outputs = fusion_model(ECG_features, me_features)
