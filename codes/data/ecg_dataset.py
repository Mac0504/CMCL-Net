...
class ECGDataset(Dataset):
    ...
    def _preprocess_data(self):
        """
        Preprocess the ECG data: normalize, filter, and encode labels.
        """
        # Normalize data
...
def load_data_split(ECG_dataset, test_size=0.2, val_size=0.1, random_state=42):
    """
    Split the dataset into training, validation, and test sets.
    """
    # Split into training+validation and test sets
    train_val_data, test_data, train_val_labels, test_labels = train_test_split(
        ECG_dataset.data, ECG_dataset.labels, test_size=test_size, random_state=random_state)
...
if __name__ == "__main__":
    # Load dataset
    ECG_dataset = ECGDataset(config['data']['ECG'])

    # Split dataset
    train_dataset, val_dataset, test_dataset = load_data_split(ECG_dataset)
