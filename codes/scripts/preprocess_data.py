...
class ECGPreprocessor:
    ...
    def bandpass_filter(self, data, lowcut, highcut):
        ...
        Args:
            data (numpy.ndarray): Input ECG data with shape (channels, samples).
    ...
    def compute_psd(self, data):
        """
        Compute the power spectral density (PSD) of the ECG data.
    ...
    def preprocess(self, ECG_data):
        """
        Perform the complete preprocessing pipeline on the ECG data.
        
        Args:
            ECG_data (numpy.ndarray): Raw ECG data with shape (channels, samples).
        
        Returns:
            numpy.ndarray: Preprocessed ECG features.
        """
        all_band_features = []
        for low, high in self.filter_bands:
            filtered_data = self.bandpass_filter(ECG_data, low, high)
...
if __name__ == "__main__":
    ...
    ECG_features = ECG_processor.preprocess(ECG_data)
