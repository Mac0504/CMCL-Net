...
train_loader, val_loader, test_loader = load_data_split(ECG_dataset, me_dataset, config['data']['split_ratio'])
...
ECG_feature_extractor.load_state_dict(torch.load(config['model']['ECG_weights']))
...
def extract_features(data_loader):
    ...
    with torch.no_grad():
        for ECG_data, me_data, labels in data_loader:
            ...
            ECG_features = ECG_feature_extractor(ECG_data)
            me_features = me_feature_extractor(me_data)
            features = torch.cat((ECG_features, me_features), dim=1)
